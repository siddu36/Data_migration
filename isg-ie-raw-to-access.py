...
def main():
    ...
    #run spark df to do transformation
    if feed_config.has_option('transformation-rules','mapping-df'):
        mapping_df = feed_config.get('transformation-rules', 'mapping-df')
        mapping_df = update_dbname_placeholders2(mapping_df,athena_raw_database,athena_access_database)
        log_manager.log(message='executing mapping df', args={'mapping_df': mapping_df, "job": JOB_KEY})
        exec(mapping_df,globals())
    #log_manager.log(message='executing mapping query', args={'mapping_sql': mapping_sql, "job": JOB_KEY})
